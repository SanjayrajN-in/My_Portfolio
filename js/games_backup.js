    ...
    if (tabButtons.length > 0) {
        ...
        tabButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                } else if (gameId === 'minesweeper-game-container') {
                    setTimeout(() => {
                        initMinesweeper();
                    }, 200);
                }
...
function initTetris() {
    ...
    function calculatePossibleMoves(row, col) {
        ...
        if (!piece || !isCurrentPlayerPiece(piece)) return moves;
    ...
    function isValidMove(fromRow, fromCol, toRow, toCol) {
        ...
        // Check if path is clear (except for knight)
        if (piece.toLowerCase() !== 'n' && !isPathClear(fromRow, fromCol, toRow, toCol)) {
            return false;
        }
        
        // Test move doesn't put own king in check
    ...
    function isPieceMovementValid(piece, fromRow, fromCol, toRow, toCol) {
        ...
        switch (piece.toLowerCase()) {
            case 'p': // Pawn
                ...
                // Diagonal capture
                if (colDiff === 1 && toRow === fromRow + direction && gameBoard[toRow][toCol]) {
                    return true;
                }
        ...
        for (let row = 0; row < 8; row++) {
            for (let col = 0; col < 8; col++) {
                ...
                if (piece && ((player === 'white' && piece === piece.toLowerCase()) || 
                    ...
                    if (canPieceAttack(piece, row, col, kingRow, kingCol, board)) {
                        return true;
                    }
                }
            }
    ...
    function canPieceAttack(piece, fromRow, fromCol, toRow, toCol, board) {
        if (!isPieceMovementValid(piece, fromRow, fromCol, toRow, toCol)) {
            return false;
        }
        
        if (piece.toLowerCase() !== 'n' && !isPathClearForBoard(fromRow, fromCol, toRow, toCol, board)) {
            return false;
        }
    ...
    // Check if path is clear for given board
    function isPathClearForBoard(fromRow, fromCol, toRow, toCol, board) {
        const rowStep = toRow === fromRow ? 0 : (toRow > fromRow ? 1 : -1);
        const colStep = toCol === fromCol ? 0 : (toCol > fromCol ? 1 : -1);
    ...
    function makeMove(fromRow, fromCol, toRow, toCol) {
        ...
        // Add visual trail for pawn moves
        ...
        if (piece.toLowerCase() === 'p') {
            if ((piece === 'P' && toRow === 0) || (piece === 'p' && toRow === 7)) {
                gameBoard[toRow][toCol] = currentPlayer === 'white' ? 'Q' : 'q';
            }
        }
        ...
        if (isCheckmate()) {
            ...
            updateMessage(`Checkmate! ${winner} wins!`);
        } else if (isStalemate()) {
            gameOver = true;
            updateMessage('Stalemate! It\'s a draw!');
        } else if (isKingInCheck(gameBoard, currentPlayer)) {
            updateMessage(`${currentPlayer === 'white' ? 'White' : 'Black'} is in check!`);
        } else {
    ...
    function isCheckmate() {
        if (!isKingInCheck(gameBoard, currentPlayer)) {
            return false;
        }
    ...
    function makeComputerMove() {
        ...
        // Adjust thinking time based on device and difficulty
        const thinkingTime = isMobile ? Math.min(500, difficulty * 100) : Math.max(100, difficulty * 50);
        
        setTimeout(() => {
            const bestMove = getBestMove(gameBoard, difficulty, 'black');
            if (bestMove) {
    ...
    function getBestMove(board, depth, player) {
        ...
        for (const move of moves) {
            const newBoard = makeTestMove(board, move);
            const value = minimax(newBoard, depth - 1, player === 'black' ? 'white' : 'black', -Infinity, Infinity);
            
            if (player === 'black' && value > bestValue) {
                bestValue = value;
                bestMove = move;
            } else if (player === 'white' && value < bestValue) {
                bestValue = value;
                bestMove = move;
            }
        }
    ...
    function minimax(board, depth, player, alpha, beta) {
        ...
        const moves = getAllValidMoves(board, player);
        if (moves.length === 0) {
            if (isKingInCheckForBoard(board, player)) {
                return player === 'black' ? -10000 : 10000;
            }
            return 0; // Stalemate
        }
        ...
        if (player === 'black') {
            ...
            for (const move of moves) {
                const newBoard = makeTestMove(board, move);
                const eval = minimax(newBoard, depth - 1, 'white', alpha, beta);
            ...
            for (const move of moves) {
                const newBoard = makeTestMove(board, move);
                const eval = minimax(newBoard, depth - 1, 'black', alpha, beta);
        ...
        for (let fromRow = 0; fromRow < 8; fromRow++) {
            for (let fromCol = 0; fromCol < 8; fromCol++) {
                ...
                if (piece && ((player === 'white' && piece === piece.toUpperCase()) || 
                    ...
                    for (let toRow = 0; toRow < 8; toRow++) {
                        for (let toCol = 0; toCol < 8; toCol++) {
                            if (isValidMoveForBoard(board, fromRow, fromCol, toRow, toCol, player)) {
                                moves.push({
                                    from: { row: fromRow, col: fromCol },
                                    to: { row: toRow, col: toCol },
                                    piece: piece,
    ...
    // Validate move for specific board and player
    function isValidMoveForBoard(board, fromRow, fromCol, toRow, toCol, player) {
        const piece = board[fromRow][fromCol];
        const targetPiece = board[toRow][toCol];
        
        // Can't capture own piece
        if (targetPiece && ((player === 'white' && targetPiece === targetPiece.toUpperCase()) || 
                           (player === 'black' && targetPiece === targetPiece.toLowerCase()))) {
            return false;
        }
        ...
        // Test move doesn't put own king in check
        const tempBoard = makeTestMove(board, {
            from: { row: fromRow, col: fromCol },
            to: { row: toRow, col: toCol }
        });
        
        if (isKingInCheckForBoard(tempBoard, player)) {
            return false;
        }
    ...
    // Make test move on board copy
    function makeTestMove(board, move) {
    ...
    // Check if king is in check for specific board
    function isKingInCheckForBoard(board, player) {
        // Find king position
        let kingRow, kingCol;
        const kingPiece = player === 'white' ? 'K' : 'k';
    ...
    function getPositionValue(piece, row, col) {
        ...
        switch (piece.toLowerCase()) {
            case 'p':
                return isWhite ? pawnTable[adjustedRow][col] : -pawnTable[adjustedRow][col];
            case 'n':
                return isWhite ? knightTable[adjustedRow][col] : -knightTable[adjustedRow][col];
            case 'b':
                return isWhite ? bishopTable[adjustedRow][col] : -bishopTable[adjustedRow][col];
            case 'r':
                return isWhite ? rookTable[adjustedRow][col] : -rookTable[adjustedRow][col];
            case 'q':
                return isWhite ? queenTable[adjustedRow][col] : -queenTable[adjustedRow][col];
            case 'k':
                return isWhite ? kingTable[adjustedRow][col] : -kingTable[adjustedRow][col];
            default:
                return 0;
        }
    }
    ...
    function updateGameInfo() {
        ...
        const modeElement = document.getElementById('chess-mode');
        const difficultyElement = document.getElementById('chess-difficulty');
        
        if (turnElement) {
            turnElement.textContent = currentPlayer === 'white' ? 'White' : 'Black';
        }
        
        if (modeElement) {
            modeElement.textContent = gameMode === 'computer' ? 'vs Computer' : 'vs Player';
        }
        
        if (difficultyElement) {
            const difficultyNames = { 1: 'Easy', 3: 'Medium', 5: 'Hard', 7: 'Expert' };
            difficultyElement.textContent = difficultyNames[difficulty] || 'Medium';
        }
    }
    
    // Update message
    function updateMessage(message) {
        const messageElement = document.getElementById('chess-message');
        if (messageElement) {
    ...
    function updateCapturedPieces() {
        ...
        if (capturedByWhite) {
            capturedByWhite.innerHTML = capturedPieces.white.map(piece => 
                `<span class="captured-piece">${getPieceSymbol(piece)}</span>`
            ).join('');
        }
        
        if (capturedByBlack) {
            capturedByBlack.innerHTML = capturedPieces.black.map(piece => 
                `<span class="captured-piece">${getPieceSymbol(piece)}</span>`
            ).join('');
        }
    }
    ...
    function undoMove() {
        ...
        // Undo computer move if in computer mode
        if (gameMode === 'computer' && moveHistory.length >= 2) {
            moveHistory.pop(); // Remove computer move
            moveHistory.pop(); // Remove player move
        } else if (gameMode === 'player' && moveHistory.length >= 1) {
        ...
        if (moveHistory.length > 0) {
            ...
            gameBoard = JSON.parse(JSON.stringify(lastMove.board));
        ...
        for (const move of moveHistory) {
            if (move.captured) {
                const capturer = move.piece === move.piece.toUpperCase() ? 'white' : 'black';
        ...
        lastMove = moveHistory.length > 0 ? {
            ...
            piece: moveHistory[moveHistory.length - 1].piece
        } : null;
        possibleMoves = [];
        updateMessage('Make your move!');
        updateGameInfo();
    ...
    // Add visual trail for piece movements
    function addMoveTrail(fromRow, fromCol, toRow, toCol) {
        // Add trail effect to squares between start and end (for multi-square moves)
        const rowStep = toRow === fromRow ? 0 : (toRow > fromRow ? 1 : -1);
        const colStep = toCol === fromCol ? 0 : (toCol > fromCol ? 1 : -1);
        ...
        // Add trail to intermediate squares
        while (currentRow !== toRow || currentCol !== toCol) {
            setTimeout(() => {
                const square = document.querySelector(`[data-row="${currentRow}"][data-col="${currentCol}"]`);
                if (square) {
                    square.classList.add('move-trail');
                    setTimeout(() => {
                        square.classList.remove('move-trail');
                    }, 2000);
                }
            }, 100);
    ...
    const resetBtn = document.getElementById('reset-chess-btn');
