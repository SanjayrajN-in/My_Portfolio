...
// @desc    Google OAuth login (legacy method - keeping for backward compatibility)
// @access  Public
...
// @desc    Login or register with Google (modern approach)
// @access  Public
...
router.get('/google/callback', async (req, res) => {
    try {
        ...
        const mode = state || 'login'; // 'login' or 'register'
        const frontendUrl = process.env.FRONTEND_URL || '/';
        
        if (!code) {
            // Redirect back to login page with error
            return res.redirect(`${frontendUrl}/pages/login.html?error=google_auth_failed&message=No authorization code received`);
        }
        ...
        if (!email_verified) {
            return res.redirect(`${frontendUrl}/pages/login.html?error=google_auth_failed&message=${encodeURIComponent('Google account email is not verified')}`);
        }
        ...
        // Create a JWT with the user data
        const userData = {
            id: user._id,
            name: user.name,
            email: user.email,
        ...
        res.redirect(`${frontendUrl}/pages/login.html?auth_success=true&token=${token}&user=${encodedUserData}`);

    } catch (error) {
        console.error('Google callback error:', error);
        res.redirect(`${frontendUrl}/pages/login.html?error=google_auth_failed&message=${encodeURIComponent('Google authentication failed')}`);
    }
});
